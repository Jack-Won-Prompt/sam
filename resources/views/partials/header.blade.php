<header x-data="{ mobileOpen: false }" class="site-header bg-white border-b border-neutral-200 sticky top-0 z-40">
    {{-- 상단 유틸 바 --}}
    ...
    {{-- 메인 헤더 --}}
    <div class="header-main container-shop flex items-center gap-6 py-4">
        ...
        <div class="flex items-center gap-5 ml-auto">
            <a id="cart-link" href="{{ route('cart.index') }}" class="relative flex flex-col items-center text-neutral-600 hover:text-brand-700 transition-colors">
                ...
                <span id="cart-count" class="absolute -top-1.5 right-1 bg-gold-500 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center {{ $cartCount > 0 ? '' : 'hidden' }}">{{ $cartCount }}</span>
            </a>
            @auth
